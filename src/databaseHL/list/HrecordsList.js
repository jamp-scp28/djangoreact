...
import Tables from "../HrecordsTables";
import Modalcont from "../ModalHL";
...
export default class HrecordsList extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async handleCreate(data, images) {
    ...
    fieldsData.append("date", data.date);
    fieldsData.append("employee", data.employee);
    fieldsData.append("date_test", data.date_test);
    fieldsData.append("result", data.result);
    fieldsData.append("date_result", data.date_result);
  ...
  async handleUpdate(data, images) {
    ...
    await queryservice
      .updateHrecord(fieldsData, data.id)
      .then((result) => {
        //        console.log(result, 'result con axios')
        let lists = [];
